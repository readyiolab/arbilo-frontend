...
const DashBoard = () => {
    ...
    useEffect(() => {
        if (!isOnline) {
            toast.error('You’re offline. The timer and data will update when you reconnect.', {
                position: 'top-center',
                autoClose: false,
            ...
            if (isWebSocketConnected && isInitialized) {
                toast.success('Reconnected! Data and timer are now synced.', {
                    position: 'top-center',
                    autoClose: 3000,
                    toastId: 'reconnect-toast'
                });
            } else if (error) {
                toast.warn('Connection issues detected. Data may be stale.', {
                    position: 'top-center',
                    autoClose: 5000,
                    toastId: 'connection-warn'
                });
            }
        }
    }, [isOnline, isWebSocketConnected, isInitialized, error]);

    // Format time for display
    const formatTime = (seconds) => {
        if (seconds == null || isNaN(seconds)) return '5m 00s';
    ...
    // Force refresh if timer reaches zero
    useEffect(() => {
        if (isInitialized && timeUntilNextRefresh <= 0 && isOnline) {
            refreshData();
        }
    }, [timeUntilNextRefresh, isInitialized, isOnline, refreshData]);
    ...
    return (
        <div className="p-2 md:p-4">
            ...
            <div className="flex flex-col items-center gap-2 mb-4">
                <div className="text-base md:text-lg font-semibold text-gray-700 transition-all duration-1000 ease-in-out">
                    Next Refresh in: <span className="inline-block transition-all duration-100">{formatTime(timeUntilNextRefresh)}</span>
                </div>
                {!isInitialized && (
